...
import { MOCK_MARKETS, MOCK_ORDER_BOOK, generatePriceFeed } from "@/lib/mockData";
...
export default function MarketDetailPage() {
  ...
  const priceFeed = generatePriceFeed(market.id);
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-yes-500/10 border border-yes-500/20 rounded-xl p-4">
          ...
          <p className="text-3xl font-bold text-yes-500">{(market.yesPrice * 100).toFixed(1)}%</p>
          <p className="text-xs text-gray-500 mt-1">{(market.yesPrice).toFixed(4)} SUI / token</p>
        </div>
        <div className="bg-no-500/10 border border-no-500/20 rounded-xl p-4">
          <p className="text-xs text-no-500 mb-1">NO Probability</p>
          <p className="text-3xl font-bold text-no-500">{(market.noPrice * 100).toFixed(1)}%</p>
          <p className="text-xs text-gray-500 mt-1">{(market.noPrice).toFixed(4)} SUI / token</p>
        </div>
      </div>
      ...
      <div className="bg-brand-500/10 border border-brand-500/20 rounded-xl p-4 mb-6 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-white">
            AI Signal: <span className="text-brand-500">{formatProbability(market.impliedProbBps)} YES</span>
          </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <PriceChart data={priceFeed} marketQuestion={market.question} />
